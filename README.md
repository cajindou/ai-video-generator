...
这是一个基于 [Taro 4](https://docs.taro.zone/docs/) + [Nest.js](https://nestjs.com/) 的 AI 探店视频生成小程序，支持上传店铺图片自动生成专业的探店视频。

## 核心功能

### 🎬 AI 视频生成
- **图片上传**：支持 3-5 张店铺图片上传
- **智能分析**：使用 AI 模型分析店铺特征（店名、产品、布局等）
- **文案生成**：自动生成高质量的口播文案
- **视频生成**：基于豆包视频生成模型创建 12 秒专业探店视频
- **视频预览**：支持在线播放和下载
...
- **工程化**: Vite 4.2.0
- **包管理**: pnpm
- **运行时**: Node.js v20.x（请使用 Node.js 20.x.x 版本，否则 better-sqlite3 会编译失败）
- **服务端**: NestJS 10.4.15
- **AI 模型**: 豆包（火山引擎）
  - 多模态模型：doubao-seed-2-0-pro-260215（图片分析、文案生成）
  - 视频生成模型：doubao-seedance-1-5-pro-251215

## 环境要求

- **Node.js**: v20.x（必须使用 Node.js 20.x.x 版本，否则 better-sqlite3 会编译失败）
- **pnpm**: 最新版本
- **Git**: 最新版本

### 部署修复说明

由于 better-sqlite3 是原生 C++ 模块，需要在编译和运行时使用相同的 Node.js 版本。如果遇到部署错误，请参考以下文档：

- [DEPLOYMENT_FIX_SUMMARY.md](./DEPLOYMENT_FIX_SUMMARY.md) - 部署错误修复总结（推荐首先查看）
...
**当前修复状态**：
- ✅ better-sqlite3 Node.js 版本兼容性：使用预编译二进制文件
- ✅ esbuild 版本冲突：使用 pnpm.overrides 强制统一版本
- ✅ 数据库文件无法打开：使用绝对路径 + 备用路径 + 错误处理
- ✅ better-sqlite3 重新编译失败：移除重新编译逻辑

**关键点**：
- 构建环境可能使用 Node.js v24，运行环境使用 Node.js v20
- 项目配置使用 better-sqlite3 的预编译二进制文件，确保版本兼容
- 使用 pnpm.overrides 强制统一 esbuild 版本，避免版本冲突
- 部署环境是只读文件系统，不能在运行时重新编译模块
- 数据库文件使用绝对路径 + 备用路径机制，确保在部署环境中正常初始化
- 移除构建阶段的重新编译逻辑，避免二次编译冲突
